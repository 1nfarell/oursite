...
function add_price_change_order(){
        ...
        if($_POST['price_change_order']){
            ...
            if ($price_change_order == 'Оплачен') {
                ...
                $st = $db_order->prepare("INSERT INTO balance_orders(number_order, sum_pay, time_pay, account) VALUES ('$number_order', '$payment_balance', '$time_pay', '$account')");
                $st->execute();
