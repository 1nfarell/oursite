...
function add_Order(){
    ...
    if(isset($_POST['number_order']) && !empty($_POST['number_order']) && isset($_POST['description_order']) && !empty($_POST['description_order']) && isset($_POST['price_order']) && !empty($_POST['price_order']) && isset($_POST['price_order--status']) && !empty($_POST['price_order--status']) && isset($_POST['adress_order']) && !empty($_POST['adress_order']) && isset($_POST['contact_order']) && !empty($_POST['contact_order']) && isset($_POST['contact_name_order']) && !empty($_POST['contact_name_order'])){
        ...
        if($sthh->rowCount() == 0){
            ...
            $array = array('number_order' => $number_order ,'description_order' => $description_order, 'price_order' => $price_order, 'price_order__status' => $price_order__status, 'today_date_order' => $today_order, 'order__status' => $order__status, 'time_last_status' => $time_last_status, 'adress_order' => $adress_order, 'contact_order' => $contact_order, 'contact_name_order' => $contact_name_order);

            $sth = $db_order->prepare("INSERT INTO orders(number_order, description_order, price_order, price_order__status, today_date_order, order__status, time_last_status, adress_order, contact_order, contact_name_order) VALUES (:number_order, :description_order, :price_order, :price_order__status, :today_date_order, :order__status, :time_last_status, :adress_order, :contact_order, :contact_name_order)");
