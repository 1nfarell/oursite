...
function get_Order(){ 
    ...
    if(strpos($search_order_in_db, "т\\")=== 0){
    ...
    // if(empty($search_order_in_db)) {    
        ...
        if (!empty($change_select_order) && !empty($change_select_user)) {
            ...
            $query .= " WHERE order__status = '$change_select_order' AND account = '$change_select_user'";
            if(!empty($search_order_in_db)){
        ...
        $query .= " ORDER BY id_order DESC LIMIT $counter_pass, $counter_element";
